...
current_NHL_game <- function(team, time_zone){
  ...
  if (status_code(response) == 200) {
    ...
    if (nrow(schedule_data$games) > 0) {
      ...
      game_information$game_state <- todays_game$gameState
      game_information$home_team_colour <- nhl_teams[nhl_teams$Team == paste(todays_game$homeTeam.placeName.default, 
                                                                             todays_game$homeTeam.commonName.default),]$Primary_Colour
      game_information$away_team_colour <- nhl_teams[nhl_teams$Team == paste(todays_game$awayTeam.placeName.default, 
                                                                             todays_game$awayTeam.commonName.default),]$Primary_Colour
      ...
      current_game_data <- get_current_NHL_game(game_information$boxscore, time_zone)
...
get_current_NHL_game <- function(boxscore, time_zone){
  ...
  # } else if (game_data$clock$running == FALSE) {
  #   game_time <- ''
  # }
  ...
  if (game_time == 'Final' | game_time == 'Scheduled') {
    ...
    period <- ifelse(game_data$periodDescriptor$number > 3, "Overtime", ifelse(!is.null(game_data$clock$Intermission), "", paste('Period:', game_data$periodDescriptor$number)))
  ...
  return(list(home_score = home_score, away_score = away_score, home_shots = home_shots, away_shots = away_shots, 
              local_time = local_time, game_time = game_time, period = period,
